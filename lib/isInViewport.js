...
(function($) {
  ...
  function isInViewport(element, options) {
    ...
    var viewportIsWindow = true;

    //if the viewport is other than window recalculate the top,
    //bottom,left and right wrt the new viewport
    //the [object DOMWindow] check is for window object type in PhantomJS
    if (typeofViewport !== '[object Window]' && typeofViewport !== '[object DOMWindow]') {
      viewportIsWindow = false;
      var $viewportOffset = $viewport.offset();

      //fetch element coords relative to document origin 
      //(previously fetched coords were relative to window)
      $element = $(element);
    ...
    //if a custom viewport is specified, we still need
    //to test against the window to ensure the element
    //is truly in view
    if(!viewportIsWindow && isVisibleFlag) {
      settings.viewport = undefined;
      isVisibleFlag = isInViewport(element, settings)
    }
